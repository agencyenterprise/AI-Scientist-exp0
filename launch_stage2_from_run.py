"""
Run only Stage 2 (baseline tuning) using a Stage 1 artifact from an existing run.

Usage:
  uv run python launch_stage2_from_run.py <run_name_or_number>

Examples:
  uv run python launch_stage2_from_run.py 3
...
def main(run_arg: str) -> None:
    ...
    # Replace/augment state with loaded Stage 1 journal and create Stage 2 stage meta
    stage1_dir = _select_stage1_dir(run_dir=run_dir)
    stage1_name, stage1_journal = _load_stage1_journal(stage1_dir=stage1_dir)

    # Ensure Stage 1 meta exists in manager.stages for seeding Stage 2
    ...
    # Create empty Stage 2 journal and meta
    ...
    # Run Stage 2 only
    manager.run_stage(
        initial_substage=stage2_meta, exec_callback=exec_callback, step_callback=step_callback
    )


if __name__ == "__main__":
    if len(sys.argv) != 2:
        logger.error("Usage: python launch_stage2_from_run.py <run_name_or_number>")
        sys.exit(1)
    main(sys.argv[1])

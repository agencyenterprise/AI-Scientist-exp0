...
def build_prompt_code_ablation(name: str, description: str, code: str, memory: str) -> str:
    ...
    - Save all data (metrics, losses, predictions, etc.) as JSON following
      ...
      ```
    - Make sure to use a filename 'experiment_data.json' to save the data. Do not
...
def build_prompt_ablation_parser_code(code: str, memory: str = "") -> str:
    ...
    `experiment_data.json` generated by an ablation study experiment.
    ...
    0. Make sure to get the working directory from `os.path.join(os.getcwd(),
       'working')`
    1. Load the `experiment_data.json` file, which is located in the working
       directory

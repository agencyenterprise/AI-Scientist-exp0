...
if TYPE_CHECKING:
    from aigraph.agents.ablation import State
...
def build_prompt_create_notes(state: "State") -> str:
    ...
    You are summarizing the results of an ablation study experiment execution. Create a concise note
    describing what was accomplished, key findings, and any important observations.

    ## Ablation Code

    <CODE>
    {state.ablation_code or "N/A"}
    </CODE>

    ## Execution Summary

    <SUMMARY>
    {state.ablation_summary or "N/A"}
    </SUMMARY>
    ...
    <STDOUT>
    {state.ablation_stdout or "N/A"}
    </STDOUT>

    <STDERR>
    {state.ablation_stderr or "N/A"}
    </STDERR>

    ## Instructions

    Write a brief note (2-3 sentences) summarizing:
    - What ablation study was performed

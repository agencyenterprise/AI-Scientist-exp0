...
class State(BaseModel):
    ...
    # generated by `node_baseline_create_experiment_plan`
...
async def node_baseline_create_experiment_plan(
    state: State, runtime: Runtime[Context]
) -> dict[str, Any]:
    logger.info("Starting node_baseline_create_experiment_plan")

    class Schema(BaseModel):
        plan: str

    prompt = prompts.build_prompt_baseline_create_plan(state.task, state.metrics)
    llms = runtime.context.llm.with_structured_output(Schema)
    response: Schema = await llms.ainvoke(prompt)  # type: ignore

    logger.debug(f"experiment_plan_structured: {response.plan[:32]!r}")
    logger.info("Finished node_baseline_create_experiment_plan")
...
async def node_baseline_code_experiment(
    ...
    prompt = prompts.build_prompt_baseline_code(
        ...
        state.experiment_plan_structured or "",
    )
...
async def node_baseline_summary(
    state: State, runtime: Runtime[Context]
) -> dict[str, Any]:
    logger.info("Starting node_baseline_summary")

    summary_state = summary.State(
        task=state.task,
        metrics=state.metrics,
        code=state.experiment_code or "",
        stdout=state.experiment_stdout or "",
        stderr=state.experiment_stderr or "",
        existing_summary=state.cumulative_summary,
        parsed_summary=state.experiment_summary or "",
    )
    ...
    logger.debug(f"cumulative_summary length: {len(state.cumulative_summary)}")
    logger.debug(f"new_summary: {new_summary[:32]!r}")

    if state.cumulative_summary:
        new_summary = f"{state.cumulative_summary}\n\n---\n\n{new_summary}"

    logger.info("Finished node_baseline_summary")
...
def build(
    ...
    builder.add_node(
        "node_baseline_create_experiment_plan",
        node_baseline_create_experiment_plan,
    )
    builder.add_node(

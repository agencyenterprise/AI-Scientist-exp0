...
class State(BaseModel):
    ...
    # generated by `node_baseline_research`
    deep_research: str = ""
...
async def node_baseline_define_metrics(
    ...
    return {"metrics": set(response.metrics)}


async def node_baseline_research(
    state: State, runtime: Runtime[Context]
) -> dict[str, Any]:
    logger.info("Starting node_baseline_research")
    ...
    research = result["final_report"]
    logger.debug(f"deep_research: {research[:32]!r}")

    logger.info("Finished node_baseline_research")
...
def build_graph_experiment(
    checkpointer: Checkpointer = None,
) -> CompiledStateGraph[State, Context, State, State]:
    """
    Build the experiment graph

    We need a separate graph because we want the deep research node to run
    concurrently throughout the whole experiment graph. This means that, when we
    start the `graph_baseline` graph, we will run the research node and the
    experiment graph in parallel.

    We cannot simply use common nodes with edges because langgraph will wait for
    both nodes to finish before moving on to the next node.
    ...
    >>> builder.add_edge(["node_b", "node_c"], END)
    >>>
    >>> graph = builder.compile()

    At first glance, the graph above seems like the nodes `node_b` will run
    concurrently with `node_c` AND `node_d`. However, this is not the case as
    langchain will wait for both, `node_b` and `node_c`, to finish before moving
    the execution to next node.

    By moving the whole flow to its own graph, we can use said graph as a node,
    and all nodes in that graph will run concurrently to the deep research node.
    """
    ...
    return builder.compile(name="graph_baseline_experiment", checkpointer=checkpointer)  # type: ignore
...
def build(
    ...
    builder.add_node(
        "node_baseline_research",
        node_baseline_research,
    )
    builder.add_node(
        "subgraph_experiment",
        build_graph_experiment(checkpointer=checkpointer),
    )
    ...
    builder.add_edge(
        ["node_baseline_research", "subgraph_experiment"],
        END,
    )

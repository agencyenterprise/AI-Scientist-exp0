...
from aigraph.agents import research_prompts
...
async def node_research(state: State, runtime: Runtime[Context]) -> dict[str, Any]:
    """Run deep research on the task."""
    logger.info("Starting node_research")

    prompt = research_prompts._task_to_prompt(state.task)
...
async def node_define_metrics(
    ...
    """Define evaluation metrics for the experiment."""
...
async def node_create_experiment_plan(
    ...
    prompt = prompts.build_prompt_prepare_plan(
        state.task, state.metrics, research=state.research or ""
    )
    llms = runtime.context.llm.with_structured_output(Schema)
    response: Schema = await llms.ainvoke(prompt)  # type: ignore
...
def build(
    ...
    # Research runs first, then metrics and plan in sequence
    builder.add_edge(START, "node_research")
    builder.add_edge("node_research", "node_define_metrics")
    builder.add_edge("node_define_metrics", "node_create_experiment_plan")
    builder.add_edge("node_create_experiment_plan", END)

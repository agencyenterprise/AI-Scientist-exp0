...
class State(BaseModel):
    ...
    tuning_directory: str | None = None
...
async def node_propose_hyperparam(state: State, runtime: Runtime[Context]) -> State:
    ...
    state.hyperparams = [
        utils.Hyperparam(name=response.name, description=response.description)
    ]
...
async def node_exec_tuning(state: State, runtime: Runtime[Context]) -> State:
    ...
    dependencies = state.tuning_deps or []
    ...
    state.tuning_directory = result.directory
...
async def node_code_metrics_parser(state: State, runtime: Runtime[Context]) -> State:
    ...
    prompt = prompts.build_prompt_code_tuning_metrics_parser(state.tuning_code or "")
...
async def node_exec_metrics_parser(state: State, runtime: Runtime[Context]) -> State:
    ...
    assert state.tuning_directory, "tuning_directory is required"

    result = await utils.exec_code_at(
        state.tuning_directory,
